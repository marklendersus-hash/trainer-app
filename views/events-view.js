...
export const renderMatchtagDetail = (callbacks) => {
    ...
    const kaderKategorieHtml = (titel, spielerListe, isAvailable = true) => `
        <div>
            <h4 class="font-bold text-lg mb-2 text-center">${titel} (${spielerListe.length})</h4>
            <div class="space-y-2">
                ${isAvailable
    ...
    return `
        <div class="p-4 rounded-xl border border-gray-700">
            ...
            <form id="matchtagForm">
                <input type="hidden" name="id" value="${state.currentId}">
                <div class="grid grid-cols-[100px,1fr] items-center gap-x-2 gap-y-4 p-3 rounded-lg bg-gray-800/50">
